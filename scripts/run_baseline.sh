#!/usr/bin/env bash
# Run the EnergyPlus FIXED-SCHEDULE baseline experiment.
# Identical IDF / EPW / occupancy / period as run_agent.sh — controller is the only difference.
set -euo pipefail
...
export RESULTS_BASELINE_DIR="${RESULTS_BASELINE_DIR:-$ROOT/results/baseline}"
mkdir -p "$RESULTS_BASELINE_DIR"
...
echo "==> Baseline EnergyPlus experiment"
echo "    IDF: $ENERGYPLUS_MODEL_PATH"
echo "    EPW: $ENERGYPLUS_WEATHER_PATH"
echo "    OUT: $RESULTS_BASELINE_DIR"
...
out = Path(os.environ["RESULTS_BASELINE_DIR"])
try:
